	...
	{#each events as event, index}
		<div class="w-full" use:addRef={event.id}>
			{#if expandedEventId === event.id}
				<!-- Expanded View -->
				<div class="flex w-full flex-col gap-3 rounded-3xl bg-black/5 p-6">
					<div class="flex items-center justify-between pb-4">
						<h2 class="cursor-pointer text-2xl font-medium md:text-3xl">{event.title}</h2>
						<button
							onclick={closeEvent}
			...
			{:else}
				...
				<div class="flex w-full {index % 2 !== 0 ? 'justify-end' : 'justify-start'}">
					<div
						class="flex w-fit max-w-full items-center gap-6 p-4 {index % 2 !== 0
							? 'flex-row-reverse text-right'
							: 'flex-row text-left'}">
						<!-- image side -->
						<button
							class="w-[33vw] shrink-0 cursor-pointer transition-opacity hover:opacity-80 focus:outline-none md:w-[25vw]"
